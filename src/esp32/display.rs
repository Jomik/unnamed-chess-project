...
impl<'d> Esp32LedDisplay<'d> {
    ...
    fn flush(&mut self) -> Result<(), LedDisplayError> {
        ...
        // send_and_wait wraps all encoders in EncoderWrapper (via into_raw),
        // which creates a Rust-allocated rmt_encoder_t and passes it to
        // rmt_transmit. BytesEncoder already holds a C-allocated encoder
        // from rmt_new_bytes_encoder — rmt_transmit rejects the Rust-side
        // wrapper with an abort in lock_acquire_generic. start_send passes
        // the original C handle directly.
        unsafe {
            self.channel
                .start_send(&mut self.encoder, &grb_bytes, &TransmitConfig::default())
                .map_err(|e| LedDisplayError::WriteFailed(format!("{e}")))?;
        }
        self.channel
            .wait_all_done(Some(core::time::Duration::from_millis(1000)))
            .map_err(|e| LedDisplayError::WriteFailed(format!("{e}")))?;
