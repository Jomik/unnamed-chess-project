...
PieceSensor::read_positions() → ByColor<Bitboard>
    → GameSession::tick(sensors) → TickResult
        ...
        → compute_feedback(position, sensors) → BoardFeedback
...
- **feedback.rs** — `compute_feedback`: pure function mapping (position, curr_sensors) → per-square LED instructions. Recovery guidance is integrated as a fallback path.
